...
router.get('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const member = await db.get('SELECT * FROM Member WHERE MemberID = ?', [memberId]);
...
router.put('/:id', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    await db.run(
      'UPDATE Member SET Name=?,Age=?,Email=?,ContactNumber=?,Department=?,YearOfStudy=?,IsActive=?,UpdatedAt=CURRENT_TIMESTAMP WHERE MemberID=?',
...
router.delete('/:id', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    await db.run('DELETE FROM Users WHERE MemberID = ?', [id]);
    await db.run('DELETE FROM Allocation WHERE MemberID = ?', [id]);
    await db.run('DELETE FROM FeePayment WHERE MemberID = ?', [id]);
    await db.run('DELETE FROM Complaint WHERE MemberID = ?', [id]);
    await db.run('DELETE FROM Visitor WHERE MemberID = ?', [id]);
    await db.run('DELETE FROM MaintenanceRequest WHERE RequestedBy = ?', [id]);
    
    // Annihilate the core member row
    await db.run('DELETE FROM Member WHERE MemberID = ?', [id]);
