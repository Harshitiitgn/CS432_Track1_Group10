...
CREATE TABLE RoomType (
    ...
    -- CONSTRAINT REMOVED due to mock data conflict
...
-- =============================================================================
-- TRIGGERS: To catch direct database modifications bypassing the API
-- =============================================================================
...
BEGIN
    ...
    VALUES ('Member', 'UPDATE', OLD.MemberID, 
            'Name:' || OLD.Name || ', Role:' || OLD.Role || ', Status:' || OLD.IsActive, 
            'Name:' || NEW.Name || ', Role:' || NEW.Role || ', Status:' || NEW.IsActive);
END;

-- Trigger for Member Deletions
CREATE TRIGGER IF NOT EXISTS trg_Member_Delete
AFTER DELETE ON Member
BEGIN
    INSERT INTO AuditLog (TableName, ActionType, RecordID, OldData, NewData)
    VALUES ('Member', 'DELETE', OLD.MemberID, 'Name:' || OLD.Name, NULL);
END;
...
BEGIN
    INSERT INTO AuditLog (TableName, ActionType, RecordID, OldData, NewData)
    VALUES ('Room', 'UPDATE', OLD.RoomID, 
            'Status:' || OLD.RoomStatus || ', Occupied:' || OLD.CurrentOccupancy, 
            'Status:' || NEW.RoomStatus || ', Occupied:' || NEW.CurrentOccupancy);
END;

-- Trigger for Allocation Inserts
CREATE TRIGGER IF NOT EXISTS trg_Allocation_Insert
AFTER INSERT ON Allocation
BEGIN
    INSERT INTO AuditLog (TableName, ActionType, RecordID, OldData, NewData)
    VALUES ('Allocation', 'INSERT', NEW.AllocationID, NULL, 
            'MemberID:' || NEW.MemberID || ', RoomID:' || NEW.RoomID);
END;

-- =============================================================================
-- TABLE: AuditLog (System-level database modifications)
-- =============================================================================
